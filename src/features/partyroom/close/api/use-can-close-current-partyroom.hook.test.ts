...
import useCanCloseCurrentPartyroom from './use-can-close-current-partyroom.hook';
...
describe('useCanCloseCurrentPartyroom', () => {
  test('me가 없으면 false를 반환한다', () => {
    const { result } = renderHook(() => useCanCloseCurrentPartyroom());
  ...
  test('MODERATOR는 파티룸을 닫을 수 없다', () => {
    store.setState({ me: { gradeType: GradeType.MODERATOR } as any });
    const { result } = renderHook(() => useCanCloseCurrentPartyroom());
    expect(result.current).toBe(false);
  });

  test('CLUBBER는 파티룸을 닫을 수 없다', () => {
