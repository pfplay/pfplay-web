    ...
    () =>
      new QueryClient({
        ...
        queryCache: new QueryCache({
          onError: (error) => tempNoticeError(error),
        }),
      })
  );
 ...
 */
function tempNoticeError(error: unknown) {
  const errorMessage = `[ERROR]\n${getErrorMessage(error)}`;

  if (typeof window === 'undefined') console.error(errorMessage);
  if (typeof window !== 'undefined') alert(errorMessage);
}
