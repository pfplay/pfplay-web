...
| Edge Config key 분기     | `production → 'maintenance'`, `preview → 'maintenance_preview'`, `development → 'maintenance_development'`  | Hobby plan 은 account 당 인스턴스 1개 — 단일 인스턴스를 prod/stg/dev 가 공유, key 로 격리. stg 점검이 prod 에 새는 사고 방지. backend 도 같은 매핑으로 write |
