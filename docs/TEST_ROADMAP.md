...
> 최초 작성: 2026-03-02 (브랜치 `test/add-mocking-tests` 기준)
> 최종 검토: 2026-05-13

## 2026-05 검토 노트

본 로드맵은 **2026-03-02 시점 스냅샷**입니다. 2026-04~05에 admin 콘솔 묶음·V13 신고 UI·V14 시스템 공지·V16 backend presence(프론트 영향 없음)·STOMP heartbeat 정정·Amplitude opt-out 등이 추가되면서 신규 테스트가 들어왔습니다.

본문 표의 수치(테스트 스위트 202 / 테스트 케이스 1003)는 그 시점 기준이며, 현재는 더 많습니다. 정확한 카운트는 `yarn test`(=`vitest run`) 출력의 마지막 요약 줄로 확인하세요. 본문의 Phase 분류·FSD 레이어별 커버리지 접근법은 여전히 유효합니다.

전수 재측정 + Phase 진행도 갱신은 별도 후속 작업으로 분리.
